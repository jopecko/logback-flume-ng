...
public class FlumeEmbeddedManager extends FlumeManager {
  ...
  public static FlumeEmbeddedManager getManager(
    ...
    return getManager(sb.toString(), factory, new FactoryData(name, agents, batchSize, dataDir));
  }

  @Override
  public void send(final Event event, final int delay, final int retries) {
    source.send(event);
  }
  ...
  private static class FactoryData {
     ...
     */
    public FactoryData(final String name,
        final List<Agent> agents,
        final int batchSize,
        final String dataDir
    ) {
